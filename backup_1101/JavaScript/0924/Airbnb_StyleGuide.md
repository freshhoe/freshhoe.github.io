...
2. 하나의 문으로 구성된 화살표 함수의 경우 중괄호를 생략하고 암시적 반환을 사용할 수 있습니다. 그 외에는 중괄호를 그대로 두고, `return` 문도 명시적으로 사용합니다. 단, 하나의 문으로 구성된 화살표 함수라도 부수효과를 일으킨다면 중괄호를 사용해줍니다. 또한 하나의 문으로 구성된 화살표 함수라도 객체 리터럴을 반환하는 경우에는 리터럴을 소괄호로 감싸거나, 중괄호로 감싼 뒤 `return` 문을 명시적으로 사용해야 합니다.
